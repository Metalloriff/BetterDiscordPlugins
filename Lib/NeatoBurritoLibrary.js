var NeatoLib = {
	...
	ContextMenu: {
		...
		createSubMenu: function(label, items, options = {}) {
			...
			element.classList.add(...this.classes.itemSubMenu.split(" "));
			...
			element.addEventListener("mouseenter", () => {
				...
				let menu = document.createElement("div");
				menu.style.left = (element.parentElement.getBoundingClientRect().right - element.parentElement.getBoundingClientRect().left + 16) + "px";
                		menu.style.top = (element.getBoundingClientRect().top - element.getBoundingClientRect().bottom) + "px";
                		menu.style.position = 'absolute';
				menu.classList.add(this.classes.contextMenu.split(" ")[0], document.getElementsByClassName("theme-dark")[0] != undefined ? "theme-dark" : "theme-light");
				for (let i = 0; i < items.length; i++) menu.appendChild(items[i]);
